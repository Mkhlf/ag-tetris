...
\author{
    ...
    \and
    Mustafa Albahrani\thanks{Joint project; however, this report is an individual submission as per the guidelines} \\
    \texttt{mustafa.albahrani@kaust.edu.sa}
}

\date{
    \today \\[1em] % Adds a little vertical space
    % Replace the URL below with your actual link
    \small \textbf{Project Repository:} \url{https://github.com/Mkhlf/ag-tetris}
}
...
Tetris is a tile-matching puzzle game originally designed by Alexey Pajitnov in 1985. The player manipulates falling geometric shapes called \textit{tetrominoes} rotating and positioning them to complete horizontal lines on a 10$\times$20 grid. Completed lines are cleared, and the game ends when pieces stack to the top of the playfield.
...
To achieve the target resolution of $1280 \times 800$ at 60Hz, our system operates with a pixel clock of approximately 83.46 MHz~\cite{tinyvga}. The visible pixels are output only during the \textit{active area}, with blanking intervals for sync pulses and porch regions.
...
\texttt{game\_control} & The core logic hub. It contains the main FSM that governs game flow (spawning, gravity, line clearing) and maintains the board state array. The board is implemented as a 10×22 grid, with 2 invisible buffer rows at the top for piece spawning, following standard Tetris convention. Only rows 2-21 are visible to the player. \\[0.5em]
...
The design was synthesized and implemented for the Artix-7 FPGA  Figure~\ref{fig:utilization} shows the resource utilization breakdown.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        % This resizes the table to fit exactly inside this subfigure
        \resizebox{\textwidth}{!}{%
            \begin{tabular}{|l|r|r|r|}
            \hline
            \textbf{Resource} & \textbf{Utilization} & \textbf{Avail.} & \textbf{Util. \%} \\ \hline
...
// The +2 offset maps visible screen coordinates to the internal 
// board array, which includes 2 hidden spawn rows at the top.
...
\item \textbf{83.46 MHz}: Pixel clock for 1280x800 VGA timing. This frequency is derived from the VESA Display Monitor Timing Standard\cite{tinyvga} which specifies 83.46 MHz as the required pixel clock for 1280$\times$800 @ 60Hz resolution.
...
\begin{thebibliography}{9}
    ...
    \bibitem{tinyvga}
    ``VESA Signal 1280 x 800 @ 60 Hz timing,'' \textit{tinyvga.com}. [Online]. Available: \url{http://www.tinyvga.com/vga-timing/1280x800@60Hz}

    \bibitem{srs}
    ``Super Rotation System,'' \textit{TetrisWiki}. [Online]. Available: \url{https://tetris.wiki/Super_Rotation_System}

    \bibitem{guideline}
    ``Tetris Guideline,'' \textit{Hard Drop Tetris Wiki}. [Online]. Available: \url{https://harddrop.com/wiki/Tetris_Guideline}

    \bibitem{rng}
    ``Random Generator,'' \textit{TetrisWiki}. [Online]. Available: \url{https://tetris.wiki/Random_Generator}
